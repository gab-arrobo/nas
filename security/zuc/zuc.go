// SPDX-FileCopyrightText: 2024 Intel Corporation
//
// SPDX-License-Identifier: Apache-2.0
//
// Specification of the 3GPP Confidentiality and Integrity Algorithms 128-EEA3 & 128-EIA3.
// Document 2: ZUC Specification
// https://www.gsma.com/security/wp-content/uploads/2019/05/eea3eia3zucv16.pdf
//

package zuc

var (
	lfsr_s [16]uint32 // the state registers of LFSR
	f_r    [2]uint32  // the registers of F
	brc_x  [4]uint32  // the outputs of bitReorganization
)

// the s-boxes
var s0 = [256]byte{
	,
}

var s1 = [256]byte{
	,
}

// the constants D
var ek_d = [16]uint32{
	0x44D7, 0x26BC, 0x626B, 0x135E, 0x5789, 0x35E2, 0x7135, 0x09AF,
	0x4D78, 0x2F13, 0x6BC4, 0x1AF1, 0x5E26, 0x3C4D, 0x789A, 0x47AC,
}

// addM function: c = a + b mod (2^31 – 1)
func addM(a, b uint32) uint32 {
	c := a + b
	return (c & 0x7FFFFFFF) + (c >> 31)
}

// mulByPow2 function: multiply by power of 2
func mulByPow2(x uint32, k int) uint32 {
	return (((x << k) | (x >> (31 - k))) & 0x7FFFFFFF)
}

// lfsrWithInitialisationMode function
func lfsrWithInitialisationMode(u uint32) {
	// Define the shifts and indices
	shifts := []int{8, 20, 21, 17, 15}
	indices := []int{0, 4, 10, 13, 15}

	// Calculate the new value of f
	f := lfsr_s[0]
	for i := 0; i < len(shifts); i++ {
		v := mulByPow2(lfsr_s[indices[i]], shifts[i])
		f = addM(f, v)
	}
	f = addM(f, u)

	// Update the state
	copy(lfsr_s[:], lfsr_s[1:])
	lfsr_s[15] = f
}

// lfsrWithWorkMode function
func lfsrWithWorkMode() {
	// Define the shifts and indices
	shifts := []int{8, 20, 21, 17, 15}
	indices := []int{0, 4, 10, 13, 15}

	// Calculate the new value of f
	f := lfsr_s[0]
	for i := 0; i < len(shifts); i++ {
		v := mulByPow2(lfsr_s[indices[i]], shifts[i])
		f = addM(f, v)
	}

	// Update the state
	copy(lfsr_s[:], lfsr_s[1:])
	lfsr_s[15] = f
}

// bitReorganization function
func bitReorganization() {
	brc_x[0] = ((lfsr_s[15] & 0x7FFF8000) << 1) | (lfsr_s[14] & 0xFFFF)
	brc_x[1] = ((lfsr_s[11] & 0xFFFF) << 16) | (lfsr_s[9] >> 15)
	brc_x[2] = ((lfsr_s[7] & 0xFFFF) << 16) | (lfsr_s[5] >> 15)
	brc_x[3] = ((lfsr_s[2] & 0xFFFF) << 16) | (lfsr_s[0] >> 15)
}

// rot function: rotate left
func rot(a uint32, k int) uint32 {
	return (a << k) | (a >> (32 - k))
}

// l1 function
func l1(X uint32) uint32 {
	return X ^ rot(X, 2) ^ rot(X, 10) ^ rot(X, 18) ^ rot(X, 24)
}

// l2 function
func l2(X uint32) uint32 {
	return X ^ rot(X, 8) ^ rot(X, 14) ^ rot(X, 22) ^ rot(X, 30)
}

// makeU32 function
func makeU32(a, b, c, d uint8) uint32 {
	return (uint32(a) << 24) | (uint32(b) << 16) | (uint32(c) << 8) | uint32(d)
}

// F function
func F() uint32 {
	W := (brc_x[0] ^ f_r[0]) + f_r[1]
	W1 := f_r[0] + brc_x[1]
	W2 := f_r[1] ^ brc_x[2]
	u := l1((W1 << 16) | (W2 >> 16))
	v := l2((W2 << 16) | (W1 >> 16))
	f_r[0] = makeU32(s0[u>>24], s1[(u>>16)&0xFF], s0[(u>>8)&0xFF], s1[u&0xFF])
	f_r[1] = makeU32(s0[v>>24], s1[(v>>16)&0xFF], s0[(v>>8)&0xFF], s1[v&0xFF])
	return W
}

// makeU31 function
func makeU31(a uint8, b uint32, c uint8) uint32 {
	return (uint32(a) << 23) | (uint32(b) << 8) | uint32(c)
}

// initialization function
func initialization(key, iv []uint8) {
	for i := 0; i < 16; i++ {
		lfsr_s[i] = makeU31(key[i], ek_d[i], iv[i])
	}

	f_r[0] = 0
	f_r[1] = 0
	for nCount := 32; nCount > 0; nCount-- {
		bitReorganization()
		w := F()
		lfsrWithInitialisationMode(w >> 1)
	}
}

// generateKeystream function
func generateKeystream(keystreamLen int) (pKeystream []uint32) {
	pKeystream = make([]uint32, keystreamLen)
	bitReorganization()
	F() // discard the output of F
	lfsrWithWorkMode()

	for i := 0; i < keystreamLen; i++ {
		bitReorganization()
		pKeystream[i] = F() ^ brc_x[3]
		lfsrWithWorkMode()
	}
	return pKeystream
}

func Zuc(key, iv []byte, length uint32) []uint32 {
	initialization(key, iv)
	stream := generateKeystream(int(length))
	return stream
}
